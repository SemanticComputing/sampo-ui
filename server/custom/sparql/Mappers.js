...
export const mapPlaces = (sparqlBindings) => {
  const results = sparqlBindings.map((b) => {
    return {
      ...
      lat: b.lat.value,
      long: b.long.value,
      ...(Object.prototype.hasOwnProperty.call(b, "instanceCount") && {
  ...
  if (sparqlBindings.length > 0) {
    results = sparqlBindings.map((b) => {
      try {
        return {
          id: b.id.value,
          prefLabel: b.prefLabel ? b.prefLabel.value : "0", // temporary prefLabel for <http://ldf.fi/MISSING_VALUE> to support sorting
...
export const mapNameSampoResults = (sparqlBindings) => {
  const results = sparqlBindings.map((b) => {
    return {
      id: b.id.value,
      prefLabel:
        b.prefLabel.value.charAt(0).toUpperCase() + b.prefLabel.value.slice(1), // capitalize
      ...
      broaderTypeLabel: has(b, "broaderTypeLabel")
        ? b.broaderTypeLabel.value
        : "-",
      collector: has(b, "collector") ? b.collector.value : "-",
      broaderAreaLabel: has(b, "broaderAreaLabel")
        ? b.broaderAreaLabel.value
        : "-",
      ...
      namesArchiveLink: has(b, "namesArchiveLink")
        ? b.namesArchiveLink.value
        : "-",
      positioningAccuracy: has(b, "positioningAccuracy")
        ? b.positioningAccuracy.value
        : "-",
      ...(Object.prototype.hasOwnProperty.call(b, "lat") && {
        lat: b.lat.value,
      }),
      ...(Object.prototype.hasOwnProperty.call(b, "long") && {
        long: b.long.value,
      }),
    };
  });
  ...
  for (let i = firstKey; i <= lastKey; i += step) {
    ...
    if (dataItemExists) {
      ...
      if (i === firstKey) {
        ...
        resultObj[i] = mergeDataItems(
          resultObj[lastNonNullIndex],
          currentDataItem,
        );
      }
      lastNonNullIndex = i;
...
export const toPolygonLayerFormat = ({ data, config }) => {
  ...
  data.forEach((item) => {
    item.choroplethColor = getChoroplethMapColor({
      value: Number(item.instanceCount),
      clusters,
    });
  });
  return data;
...
const getChoroplethMapColor = ({ value, clusters }) => {
  ...
  colors.forEach((color, index) => {
    if (
      value >= Number(clusters[index][0]) &&
      value <= Number(clusters[index][clusters[index].length - 1])
    ) {
      heatmapColor = color;
    }
  });
  ...
  data.forEach((item) => {
    ...
    target.forEach((targetItem) => {
      ...
      if (hours == null || minutes == null || seconds == null) {
        ...
        const paddedTimecode = createPaddedTimeCode({
          hours,
          minutes,
          seconds,
        });
...
export const createCorrespondenceChartData = ({ sparqlBindings, config }) => {
  ...
  const cnAll = new Counter(
    sparqlBindings.map((ob) => ob[ob.type + "__label"]),
  );
...
export const createCorrespondenceChartDataLower = ({
  sparqlBindings,
  config,
}) => {
  const series = [];
  Object.entries(mapMultipleLineChart({ sparqlBindings, config })).forEach(
    ([key, arr]) => {
